...
class ilCertificateConfigFormGUI extends ilPropertyFormGUI
{
    ...
    protected function initForm()
    {
        ...
        // Course templates
        $item = new ilCheckboxInputGUI($this->txt(ilCertificateConfig::COURSE_TEMPLATES),
            ilCertificateConfig::COURSE_TEMPLATES);
        $item->setInfo($this->txt(ilCertificateConfig::COURSE_TEMPLATES . '_info'));
        $subitem = new ilTextAreaInputGUI($this->txt(ilCertificateConfig::COURSE_TEMPLATES_REF_IDS),
            ilCertificateConfig::COURSE_TEMPLATES_REF_IDS);
        $subitem->setInfo($this->txt(ilCertificateConfig::COURSE_TEMPLATES_REF_IDS . '_info'));
        $item->addSubItem($subitem);
        $this->addItem($item);

        // UTC
        $item = new ilCheckboxInputGUI($this->txt(ilCertificateConfig::TIME_FORMAT_UTC),
            ilCertificateConfig::TIME_FORMAT_UTC);
        ...
        // Datetime format
        $item = new ilTextInputGUI($this->txt(ilCertificateConfig::DATETIME_FORMAT),
            ilCertificateConfig::DATETIME_FORMAT);
        $item->setInfo($this->txt(ilCertificateConfig::DATETIME_FORMAT . '_info'));
        $item->setRequired(true);
        $this->addItem($item);

        // Max diff LP seconds
        $item = new ilNumberInputGUI($this->txt(ilCertificateConfig::MAX_DIFF_LP_SECONDS),
            ilCertificateConfig::MAX_DIFF_LP_SECONDS);
        $item->setInfo($this->txt(ilCertificateConfig::MAX_DIFF_LP_SECONDS . '_info'));
        $this->addItem($item);

        // Hook class
        $item = new ilTextInputGUI($this->txt(ilCertificateConfig::PATH_HOOK_CLASS),
            ilCertificateConfig::PATH_HOOK_CLASS);
        $item->setInfo(sprintf($this->txt(ilCertificateConfig::PATH_HOOK_CLASS . '_info'),
            ilCertificatePlugin::CLASS_NAME_HOOKS, ilCertificatePlugin::DEFAULT_PATH_HOOK_CLASS));
        $this->addItem($item);

        //Call Back email
        $item = new ilTextInputGUI($this->txt(ilCertificateConfig::CALLBACK_EMAIL),
            ilCertificateConfig::CALLBACK_EMAIL);
        $item->setInfo($this->txt(ilCertificateConfig::CALLBACK_EMAIL . '_info'));
        $this->addItem($item);

        //disk space Warning
        $item = new ilTextInputGUI($this->txt(ilCertificateConfig::DISK_SPACE_WARNING),
            ilCertificateConfig::DISK_SPACE_WARNING);
        ...
        // Administrate types
        $item = new ilMultiSelectInputGUI($this->txt(ilCertificateConfig::ROLES_ADMINISTRATE_CERTIFICATE_TYPES),
            ilCertificateConfig::ROLES_ADMINISTRATE_CERTIFICATE_TYPES);
        ...
        $item = new ilMultiSelectInputGUI($this->txt(ilCertificateConfig::ROLES_ADMINISTRATE_CERTIFICATES),
            ilCertificateConfig::ROLES_ADMINISTRATE_CERTIFICATES);
        $item->setOptions($roles);
        ...
        $item = new ilTextInputGUI($this->txt(ilCertificateConfig::JASPER_JAVA_PATH),
            ilCertificateConfig::JASPER_JAVA_PATH);
        $this->addItem($item);
        ...
        $item = new ilTextInputGUI($this->txt(ilCertificateConfig::NOTIFICATIONS_USER_SUBJECT),
            ilCertificateConfig::NOTIFICATIONS_USER_SUBJECT);
        $this->addItem($item);

        $item = new ilTextAreaInputGUI($this->txt(ilCertificateConfig::NOTIFICATIONS_USER_BODY),
            ilCertificateConfig::NOTIFICATIONS_USER_BODY);
        $item->setRows(10);
        $item->setCols(100);
        $this->addItem($item);

        $item = new ilTextInputGUI($this->txt(ilCertificateConfig::NOTIFICATIONS_OTHERS_SUBJECT),
            ilCertificateConfig::NOTIFICATIONS_OTHERS_SUBJECT);
        $this->addItem($item);

        $item = new ilTextAreaInputGUI($this->txt(ilCertificateConfig::NOTIFICATIONS_OTHERS_BODY),
            ilCertificateConfig::NOTIFICATIONS_OTHERS_BODY);
        $item->setRows(10);
        $item->setCols(100);
        $this->addItem($item);
